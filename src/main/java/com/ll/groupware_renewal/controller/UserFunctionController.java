...
import com.ll.groupware_renewal.config.UserFunctionConfig;
...
public class UserFunctionController {
    ...
    private final UserFunctionConfig userFunctionConfig;
    ...
    public String findPassword() {
        return userFunctionConfig.getUrls().getFindPassword().toString();
    }
    ...
    public String showPassword(User user, RedirectAttributes redirectAttributes, Model model,
        ...
        return this.userFunctionConfig.getUrls().getShowPassword().toString();
    }
    ...
    public String home(User user, Principal principal, Model model, HttpServletRequest request, Student student,
        ...
        if (principal != null) { // 지우면 오류(로그인 안해서 principal 못가져오는)납니다
            ...
            if (SelectUserProfileInfo.get(2).equals(this.userFunctionConfig.getString().getStudent())) {
                ArrayList<String> StudentInfo = new ArrayList<String>();
                StudentInfo = studentService.SelectStudentProfileInfo(SelectUserProfileInfo.get(1));

                userInfoMethod.StudentInfo(model, SelectUserProfileInfo, StudentInfo);
            } else if (SelectUserProfileInfo.get(2).equals(this.userFunctionConfig.getString().getProfessor())) {
            ...
            } else if (SelectUserProfileInfo.get(2).equals(this.userFunctionConfig.getString().getAdmin())) {
                userInfoMethod.AdministratorInfo(model, SelectUserProfileInfo);
            }
            Date Now = new Date();
            SimpleDateFormat Date = new SimpleDateFormat(this.userFunctionConfig.getFormat().getDate().toString());
        ...
        model.addAttribute(this.userFunctionConfig.getString().getNoticeList().toString(), NoticeList);

        // 커뮤니티 리스트 띄우기
        List<Board> CommunityList = boardService.SelectCommunityBoardList();
        model.addAttribute(this.userFunctionConfig.getString().getCommunityList().toString(), CommunityList);

        return this.userFunctionConfig.getUrls().getHomeView().toString();
    }
    ...
    public String myPageByRole(HttpServletRequest request, Model model) throws IOException {
        ...
        if (MysqlRole.equals(this.userFunctionConfig.getString().getStudent())) {
            return this.userFunctionConfig.getUrls().getReMypageStudent().toString();
        } else if (MysqlRole.equals(this.userFunctionConfig.getString().getProfessor())) {
            return this.userFunctionConfig.getUrls().getReMypageProfessor().toString();
        } else if (MysqlRole.equals(this.userFunctionConfig.getString().getAdmin())) {
            return this.userFunctionConfig.getUrls().getReHome().toString();
        }
        return this.userFunctionConfig.getUrls().getReHome().toString();
    }
    ...
    public String myPostList(Model model, Principal principal, User user) {
        ...
        model.addAttribute(this.userFunctionConfig.getString().getMyBoardList().toString(), MyBoardList);

        return this.userFunctionConfig.getUrls().getMyPostList().toString();
    }
    ...
    public String myInquiryList(Model model, Principal principal, User user) {
        ...
        if (!MyInquiryList.isEmpty()) {
            model.addAttribute(this.userFunctionConfig.getString().getMyInquiryList().toString(), MyInquiryList);
        }
        return this.userFunctionConfig.getUrls().getMyInquiryList().toString();
    }
    ...
    public String checkMyTeam(Model model, Principal principal, User user) {
        // 유저 정보
        GetUserInformation(principal, user, model);
        return this.userFunctionConfig.getUrls().getCheckMyTeam().toString();
    }

    /* 정보 수정 버튼 클릭 시 비밀번호 확인 */
    @RequestMapping(value = "/checkPassword", method = RequestMethod.GET)
    public String checkPassword() {
        return this.userFunctionConfig.getUrls().getCheckPassword().toString();
    }

    /* 비밀번호 변경 화면 */
    @RequestMapping(value = "/modifyPassword", method = RequestMethod.GET)
    public String modifyPassword() {
        return this.userFunctionConfig.getUrls().getModifyPassword().toString();
    }

    // 탈퇴 매뉴얼
    @RequestMapping(value = "/withdrawal", method = RequestMethod.GET)
    public String withdrawal() {
        return userFunctionConfig.getUrls().getWithdrawal().toString();
    }
    ...
    public String DoWithdrawl(HttpServletRequest request, Principal Principal, User user, Student student,
        ...
        if ((String) request.getParameter(userFunctionConfig.getFields().getAgreeWithdrawal().toString()) != null) {
            // user정보 select해서 user에 set
            User UserInfo = userService.SelectUserInfo(UserLoginID);
            user.setUserLoginID(UserInfo.getUserLoginID());
            // 탈퇴한 날짜 set
            Date Now = new Date();
            SimpleDateFormat Date = new SimpleDateFormat(userFunctionConfig.getFormat().getDate().toString());
            user.setDate(Date.format(Now));
    ...
    public String checkPassword2() {
        return this.userFunctionConfig.getUrls().getCheckPassword2().toString();
    }
    ...
    public String checkPassword2(HttpServletResponse response, HttpServletRequest request, Principal Principal)
        ...
        boolean Checker = userService.SelectForPwdCheckBeforeModify(UserID,
                (String) request.getParameter(this.userFunctionConfig.getFields().getUserPassword().toString()));
        if (Checker == true) {
            return this.userFunctionConfig.getUrls().getReWithdrawal().toString();
        } else {
    ...
    public String emailAuthentication() {
        return this.userFunctionConfig.getUrls().getEmailVerification().toString();
    }
    ...
    public String DoEmail(User user, UserEmail userEmail, RedirectAttributes redirectAttributes, Model model,
                          HttpServletRequest request, HttpServletResponse response) throws IOException {
        UserEmail = (String) request.getParameter(userFunctionConfig.getFields().getUserEmail().toString());
        if ((String) request.getParameter(userFunctionConfig.getFields().getUserEmail().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getUserEmail().toString(), UserEmail);
            Address = userFunctionConfig.getString().getEmailDomain().toString();
            UserEmail = UserEmail + Address;
            user.setUserEmail(UserEmail);
        }
        if ((String) request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getAuthNum().toString(),
                    (String) request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()));
        }
        if (request.getParameter(userFunctionConfig.getFields().getEmailCheck().toString()) != null && !UserEmail.equals("")) {
            user.setUserEmail(UserEmail);
            // 이메일 중복확인
            ...
            if (!EmailCheck) {
                ...
                DateFormat = new SimpleDateFormat(userFunctionConfig.getFormat().getDateTime().toString());
                Now = new Date();
            ...
            return userFunctionConfig.getUrls().getEmailVerification().toString();
        } else if (UserEmail.equals("")) {
            // 이메일을 입력해주세요
        } else if (request.getParameter(userFunctionConfig.getFields().getEmailValid().toString()) != null
                && request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()) != "") {
            boolean Checker = userEmailService
                    .SelectForCheckCertification(request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()));
            if (Checker) {
                response.setContentType("text/html; charset=UTF-8");
                PrintWriter Out = response.getWriter();
        ...
        if (request.getParameter(userFunctionConfig.getFields().getBtnAgree().toString()) != null && EmailChecker) {
            return userFunctionConfig.getUrls().getSignupSelect().toString();
        } else {
            return userFunctionConfig.getUrls().getEmailVerification().toString();
        }
    }
    ...
    public String DoSignUp(User user, Student student, RedirectAttributes redirectAttributes, Model model,
        ...
        if ((String) request.getParameter(userFunctionConfig.getFields().getUserPassword().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getUserPassword().toString(), (String) request.getParameter(userFunctionConfig.getFields().getUserPassword().toString()));
        }
        if ((String) request.getParameter(userFunctionConfig.getFields().getStudentName().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getStudentName().toString(), (String) request.getParameter(userFunctionConfig.getFields().getStudentName().toString()));
        }
        ...
        if ((String) request.getParameter("UserPhoneNum") != null) {
            model.addAttribute(userFunctionConfig.getFields().getUserPhoneNum().toString(),
                    (String) request.getParameter(userFunctionConfig.getFields().getUserPhoneNum().toString()));
        }
        if ((String) request.getParameter(userFunctionConfig.getFields().getStudentNum().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getStudentNum().toString(), request.getParameter(userFunctionConfig.getFields().getStudentNum().toString()));
        }
        ...
        if (request.getParameter("IdCheck") != null) {
            ...
            if (UserLoginID.equals("")) {
                ...
                Out.flush();
                return userFunctionConfig.getUrls().getSignupStudent().toString();
            ...
            if (StudentColleges.equals("")) {
                ...
                user.setUserRole(userFunctionConfig.getString().getStudent().toString()); // user role = 학생
                user.setUserEmail(UserEmail);
                ...
                Out.flush();
                return userFunctionConfig.getUrls().getLogin().toString();
            }
        } else {
            return userFunctionConfig.getUrls().getSignupStudent().toString();
        }
    ...
    public String dosignup(User user, Professor professor, RedirectAttributes redirectAttributes, Model model,
        ...
        if ((String) request.getParameter(userFunctionConfig.getFields().getProfessorName().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getProfessorName().toString(), (String) request.getParameter(userFunctionConfig.getFields().getProfessorName().toString()));
        }
        if ((String) request.getParameter("UserPhoneNum") != null) {
            model.addAttribute(userFunctionConfig.getFields().getUserPhoneNum().toString(),
                    (String) request.getParameter(userFunctionConfig.getFields().getUserPhoneNum().toString()));
        }
        if ((String) request.getParameter(userFunctionConfig.getFields().getProfessorNum().toString()) != null) {
            model.addAttribute(userFunctionConfig.getFields().getProfessorNum().toString(), request.getParameter(userFunctionConfig.getFields().getProfessorNum().toString()));
        }
        if ((String) request.getParameter("UserColleges") != null) {
        ...
        if (request.getParameter("IdCheck") != null) {
            ...
            if (ProfessorColleges.equals("")) {
                ...
                user.setUserRole(userFunctionConfig.getString().getProfessor().toString()); // user role = 교수
                user.setUserEmail(UserEmail);
    ...
    public String findPassword(User user, RedirectAttributes redirectAttributes, Model model,
        ...
        if (request.getParameter("IdCheck") != null) {
            user.setUserLoginID(UserLoginID);
            user.setUserName((String) request.getParameter(userFunctionConfig.getFields().getUserName().toString()));
            if (UserLoginID.equals("")) {
                response.setContentType("text/html; charset=UTF-8");
                PrintWriter Out = response.getWriter();
                Out.println("<script>alert('계정을 입력하지 않으셨습니다.');</script>");
                Out.flush();
            } else if (((String) request.getParameter(userFunctionConfig.getFields().getUserName().toString())).equals("")) {
                model.addAttribute("UserLoginID", UserLoginID);
                response.setContentType("text/html; charset=UTF-8");
                PrintWriter Out = response.getWriter();
            ...
            if (IDChecker) {
                model.addAttribute("UserLoginID", UserLoginID);
                model.addAttribute(userFunctionConfig.getFields().getUserName().toString(),
                        (String) request.getParameter(userFunctionConfig.getFields().getUserName().toString()));
                response.setContentType("text/html; charset=UTF-8");
                PrintWriter Out = response.getWriter();
            ...
            model.addAttribute(userFunctionConfig.getFields().getUserName().toString(),
                    (String) request.getParameter(userFunctionConfig.getFields().getUserName().toString()));
            model.addAttribute("UserEmail", UserEmail);
            NameChecker = emailService.AuthNum((String) request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()));
            if (NameChecker) {
                model.addAttribute(userFunctionConfig.getFields().getAuthNum().toString(),
                        (String) request.getParameter(userFunctionConfig.getFields().getAuthNum().toString()));
                response.setContentType("text/html; charset=UTF-8");
            ...
            return userFunctionConfig.getUrls().getShowPassword().toString();
        }
        return userFunctionConfig.getUrls().getFindPassword().toString();
    }
    ...
    public String checkPassword(HttpServletResponse response, HttpServletRequest request, Principal Principal)
        ...
        boolean Checker = userService.SelectForPwdCheckBeforeModify(UserLoginID,
                (String) request.getParameter(userFunctionConfig.getFields().getUserPassword().toString()));
        String MysqlRole = userService.SelectUserRole(UserLoginID);
        if (Checker == true) {
            if (MysqlRole.equals(userFunctionConfig.getString().getStudent())) {
                return userFunctionConfig.getUrls().getReModifyPassword().toString();
            } else if (MysqlRole.equals(userFunctionConfig.getString().getProfessor())) {
                return userFunctionConfig.getUrls().getReModifyProfessor().toString();
            }
        } else {
            response.setContentType("text/html; charset=UTF-8");
            PrintWriter Out = response.getWriter();
            Out.println("<script>alert('비밀번호를 다시 입력해주세요.' );</script>");
            Out.flush();
            return userFunctionConfig.getUrls().getCheckPassword().toString();
        }
        return "/";
    }
    ...
    public String checkPassword3() {
        return userFunctionConfig.getUrls().getCheckPassword3().toString();
    }
    ...
    public String modifyPassword(HttpServletResponse response, HttpServletRequest request, User user,
        ...
        String UserLoginID = Principal.getName();
        String HashedPwd = BCrypt.hashpw((String) request.getParameter(userFunctionConfig.getFields().getUserNewPassword().toString()),
                BCrypt.gensalt());
        user.setUserModifiedPW(HashedPwd);
        // 새로입력한 비밀번호와 확인 비밀번호가 일치하면
        if ((request.getParameter(userFunctionConfig.getFields().getUserNewPassword().toString())
                .equals((String) request.getParameter(userFunctionConfig.getFields().getUserNewPasswordCheck().toString())))) {
            String UserLoginPwd = userService.SelectCurrentPwd(UserLoginID);
            user.setUserLoginPwd(UserLoginPwd);
            userService.UpdatePwd(user);

            return userFunctionConfig.getUrls().getModifyPassword().toString();
        }
        return userFunctionConfig.getUrls().getModifyPassword().toString();
    }
    ...
    public String emailLogin() {
        return userFunctionConfig.getUrls().getEmailLogin().toString();
    }
    ...
    public String DoEmailLogin(HttpServletRequest request, Model model, Principal principal, User user,
        ...
        String ID = request.getParameter("EmailLoginID") + userFunctionConfig.getString().getEmailDomain();

        boolean CheckID = emailService.CheckEmailLogin(ID, request.getParameter(userFunctionConfig.getFields().getEmailPassword().toString()));
        if (CheckID) {
            return userFunctionConfig.getUrls().getReEmailList().toString();
        } else {
            rttr.addFlashAttribute("Checker", "LoginFail");
            return userFunctionConfig.getUrls().getReEmailLogin().toString();
        }
    }
    ...
    public String emailList(HttpServletRequest request, Model model, Principal principal, User user) {
        ...
        if (EmailList.isEmpty()) {
            return userFunctionConfig.getUrls().getEmailList().toString();
        } else {
    ...
    public String emailContent(HttpServletRequest request, Model model, Principal principal, User user) {
        ...
        return userFunctionConfig.getUrls().getEmailContent().toString();
    }
